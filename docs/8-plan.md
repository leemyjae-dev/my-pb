...
| v1.1 | 2026-09-16 | DB-1~3, BE-1~8, FE-1~9 전체 완료 확인 — 88개 완료 조건 체크 |
...
**완료 조건**
- [x] PostgreSQL 17 인스턴스에 정상 접속 가능
- [x] `my_pb` 데이터베이스가 생성되어 있음 (실제로는 기존 `postgres` 데이터베이스를 그대로 사용함 — 이름만 다르고 동일 역할)
- [x] `DATABASE_URL` 접속 문자열을 확보함 (실제 `.env` 키 이름은 `POSTGRES_CONNECTION_STRING`)
...
- 해당 SQL을 `my_pb`(실제로는 `postgres`) 데이터베이스에 실행하여 8개 테이블 생성
- 생성된 테이블·FK 제약조건·인덱스가 `docs/7-erd.md`와 일치하는지 확인

**완료 조건**
- [x] `schema.sql`이 `backend/src/db/migrations/`에 위치함
- [x] `users`, `asset_classes`, `kyc_profile_results`, `neuroticism_test_results`, `holdings_snapshots`, `holding_items`, `target_asset_allocations`, `risk_diagnosis_results` 8개 테이블이 모두 생성됨
- [x] FK 제약조건과 인덱스가 정상 생성됨을 확인함 (`\d` 등으로 확인)
...
**완료 조건**
- [x] ⚠️ **선결 필요**: 5단계 성향 등급별 자산군 기준 비중표가 확정됨 (도메인 정의서 8절 — 미확정 시 도메인 정의서 예시 1건만으로 전체 표를 임시 구성해 진행)
- [x] 자산군 목록이 확정되어 `asset_classes`에 입력 완료됨
- [x] 확정된 비중표 전체 조합이 `target_asset_allocations`에 입력 완료됨
...
**완료 조건**
- [x] `npm install`로 필요한 패키지 설치 완료
- [x] `5-project-principle.md` 6절 구조대로 디렉토리가 생성됨
- [x] `server.js` 실행 시 지정한 `PORT`로 서버가 정상 기동됨
- [x] `db/pool.js`를 통해 DB-1의 PostgreSQL에 커넥션 성공 확인
- [x] `.env.example`에 필요한 환경변수 키가 모두 나열됨
...
**완료 조건**
- [x] access_token 없이 보호된 라우트 요청 시 401 응답 확인
- [x] 유효한 access_token으로 요청 시 정상 통과 확인
- [x] 처리되지 않은 에러 발생 시 공통 에러 응답 포맷으로 반환됨을 확인
- [x] 허용되지 않은 origin의 요청이 CORS로 차단됨을 확인
...
**완료 조건**
- [x] `POST /api/auth/signup` — 이메일/비밀번호로 회원가입 성공 (FR-0.1)
- [x] 중복 이메일로 회원가입 시도 시 오류 응답 반환 확인
- [x] `POST /api/auth/login` — 올바른 자격증명으로 `access_token`+`refresh_token` 발급 확인 (FR-0.2, FR-0.4)
- [x] 잘못된 자격증명으로 로그인 시 401 반환 확인
- [x] `POST /api/auth/refresh` — `refresh_token`으로 새 `access_token` 발급 확인 (FR-0.4)
- [x] 비밀번호가 DB에 평문이 아닌 해시로 저장됨을 확인
...
**완료 조건**
- [x] KYC 채점 함수 단위 테스트 작성 및 통과 (구간 경계값 0/19/20/39/40/59/60/79/80/100 포함)
- [x] `POST /api/kyc` — 10문항 응답 제출 시 총점·등급 산출 후 저장 확인 (FR-1.1~FR-1.3)
- [x] 인증되지 않은 요청은 401 반환 확인
- [x] 재응시 시 기존 결과를 덮어쓰지 않고 새 결과가 별도 생성됨 확인 (FR-1.4)
- [x] `GET /api/kyc/latest` — 가장 최근 결과 조회 확인
...
**완료 조건**
- [x] IPIP 채점 함수(역채점 반전 포함) 단위 테스트 작성 및 통과
- [x] `POST /api/neuroticism` — 10문항 응답 제출 시 총점·심리민감도 등급 산출 후 저장 확인 (FR-2.1~FR-2.4)
- [x] 재응시 시 새 결과가 별도 생성됨 확인 (FR-2.5)
- [x] `GET /api/neuroticism/latest` — 최근 결과 조회 확인
...
**완료 조건**
- [x] ⚠️ **선결 필요**: 이미지 인식(OCR/AI 비전) 구현 수단이 확정됨 (도메인 정의서 8절, PRD 9절)
- [x] ⚠️ **선결 필요**: 업로드 이미지 저장 위치/보관 정책이 확정됨 (도메인 정의서 8절)
- [x] `POST /api/holdings` — 이미지 업로드 시 `holdings_snapshots` 레코드 생성 확인 (FR-3.1)
- [x] 업로드된 이미지에서 종목명·수량·평가금액이 인식되어 `holding_items`로 저장됨 확인 (FR-3.2)
- [x] 허용되지 않은 파일 형식/크기 초과 업로드 시 거부됨 확인
...
**완료 조건**
- [x] ⚠️ **선결 필요**: 종목명→자산군 자동 분류 기준 데이터가 확정됨 (도메인 정의서 8절)
- [x] 인식된 종목이 자산군으로 자동 분류되어 저장됨 확인 (FR-3.3)
- [x] `PATCH /api/holdings/{snapshotId}/items/{itemId}` — 사용자가 자산군을 직접 수정 가능함 확인 (FR-3.4)
- [x] `POST /api/holdings/{snapshotId}/confirm` — 보정 필요 항목이 남아있으면 확정이 거부됨 확인
- [x] 모든 항목이 확정 상태일 때 자산군별 비중이 계산되어 저장됨 확인 (FR-3.5)
- [x] 재업로드 시 새 스냅샷이 별도 생성됨 확인 (FR-3.6)
...
**완료 조건**
- [x] ⚠️ **선결 필요**: 정합성/괴리 판정 임계값이 확정됨 (도메인 정의서 8절)
- [x] 정합/괴리 판정 함수 단위 테스트 작성 및 통과 (경계값 포함)
- [x] `POST /api/diagnosis` — 3종 입력 중 하나라도 없으면 부족한 항목을 안내하는 응답 반환 확인 (FR-4.6)
- [x] 3종 입력이 모두 있으면 자산군별 정합/괴리 판정 결과 반환 확인 (FR-4.2)
- [x] 심리민감도 '높음' + 괴리 결합 시 심리적 부담 안내가 결과에 포함됨 확인 (FR-4.3)
- [x] 정합/괴리 각각에 맞는 피드백 메시지가 생성됨 확인 (FR-4.4)
- [x] 진단 결과가 `risk_diagnosis_results`에 저장되고 응답으로 반환됨 확인 (FR-4.5)
...
**완료 조건**
- [x] ⚠️ **선결 필요**: 프론트엔드 라우팅 라이브러리가 확정됨 (`5-project-principle.md` 확인 필요 항목)
- [x] React 19 + TypeScript 프로젝트 생성 및 로컬 실행 확인
- [x] 라우팅 라이브러리 설치 및 기본 라우트(빈 페이지 기준) 동작 확인
- [x] `zustand`, `@tanstack/react-query` 설치 및 `QueryClientProvider`가 `App.tsx`에 적용됨 확인
- [x] `frontend/src` 하위 `app/pages/features/entities/shared` 디렉토리 생성됨
- [x] `shared/api/httpClient.ts` 작성 및 기본 요청 동작 확인
...
**완료 조건**
- [x] 공통 Button/Input 컴포넌트가 2개 이상 화면에서 재사용됨 확인
- [x] 인증이 필요한 API 요청 시 `Authorization` 헤더가 자동으로 첨부됨 확인
- [x] `access_token` 만료 시 `refresh_token`으로 재발급 후 원 요청이 자동 재시도됨 확인 (사용자 시나리오 7)
...
**완료 조건**
- [x] 회원가입 화면에서 이메일/비밀번호 입력 후 가입 성공 시 로그인 화면으로 이동 확인 (시나리오 1)
- [x] 로그인 성공 시 토큰이 저장되고 대시보드로 이동 확인
- [x] 로그인 실패/회원가입 오류 시 화면에 오류 메시지가 표시됨 확인
- [x] 비로그인 상태에서 보호된 화면 접근 시 로그인 화면으로 리다이렉트됨 확인 (FR-0.3)
...
**완료 조건**
- [x] 10개 문항에 모두 응답해야 제출 가능함(또는 미응답 시 안내) 확인
- [x] 제출 시 결과가 저장되고 대시보드로 복귀함 확인 (시나리오 2)
- [x] 데스크톱/모바일 반응형 레이아웃 확인
...
**완료 조건**
- [x] 10개 문항 응답 제출 시 결과가 저장되고 대시보드로 복귀함 확인 (시나리오 3)
- [x] 데스크톱/모바일 반응형 레이아웃 확인
...
**완료 조건**
- [x] 업로드 화면에서 이미지 선택 후 업로드 시 처리 중 상태가 표시됨 확인
- [x] 업로드 완료 후 자산 항목 확인/보정 화면으로 자동 이동 확인
- [x] 보정 필요 항목의 자산군을 드롭다운 등으로 수정 가능함 확인 (시나리오 4)
- [x] 보정 필요 항목이 남아있는 상태로 확정 시도 시 안내 메시지가 표시되고 확정이 차단됨 확인
- [x] 모든 항목 확정 완료 후 대시보드로 이동 확인
- [x] 데스크톱/모바일 반응형 레이아웃 확인
...
**완료 조건**
- [x] 로그인 후 대시보드에 3종 입력 상태 카드(완료 여부/등급/일시)가 표시됨 확인
- [x] 각 카드의 버튼 클릭 시 해당 입력 화면으로 이동 확인
- [x] [종합 진단 결과 확인] 버튼 클릭 시 결과 화면으로 이동 확인
- [x] [로그아웃] 클릭 시 인증 상태가 초기화되고 로그인 화면으로 이동 확인
...
**완료 조건**
- [x] 입력 누락 상태일 때 부족한 항목 체크리스트와 이동 버튼이 표시됨 확인 (시나리오 6)
- [x] 3종 입력 완료 후 자산군별 성향기준/실보유 비중 비교 표(또는 카드)가 표시됨 확인
- [x] 괴리인 경우 조정 방향 피드백 메시지가 표시됨 확인
- [x] 심리민감도 '높음' + 괴리 결합 시 심리적 부담 안내가 함께 표시됨 확인
- [x] 전 자산군 정합인 경우 긍정 피드백 메시지가 표시됨 확인
- [x] 데스크톱/모바일 반응형 레이아웃 확인
...
**완료 조건**
- [x] 8개 화면 전체가 데스크톱/모바일 주요 화면폭에서 레이아웃 깨짐 없이 표시됨
- [x] 전체 플로우(회원가입→로그인→3종 입력→종합 진단)를 처음부터 끝까지 수동으로 완주함
- [x] 시나리오 6(입력 누락), 시나리오 7(토큰 만료 재인증) 예외 흐름이 정상 동작함을 확인
